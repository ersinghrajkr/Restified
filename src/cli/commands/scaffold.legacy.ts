...
const config: RestifiedConfig = {
  ...
  authentication: {
    ...
    credentials: {
      username: process.env.AUTH_USERNAME || 'admin@company.com',
...
function generateAuthTest(options: any): string {
  return `/**
 * ${options.name} JWT Authentication Tests
 * 
 * Comprehensive JWT authentication testing with proper login flow
 */
...
describe('${options.name} Authentication Tests', function() {
  ...
  describe('JWT Authentication Flow', function() {
    it('should perform JWT login with credentials', async function() {
      const loginPayload = {
      ...
      await response
        ...
        .extract('$.token', 'jwtToken')
        .extract('$.user.email', 'userEmail')  
        .extract('$.user.id', 'userId')
        .extract('$.expiresIn', 'tokenExpiry')
        .execute();

      // Verify extracted JWT data
      const jwtToken = restified.getVariable('jwtToken');
      const userEmail = restified.getVariable('userEmail');
      const userId = restified.getVariable('userId');
    ...
    it('should use JWT token for authenticated requests', async function() {
      // JWT token should already be applied by global setup
      const response = await restified
        .given()
          .useClient('api')
          .header('Authorization', \`Bearer $\{restified.getVariable('globalAuthToken')}\`)
        .when()
          .get('/users/profile')
          .execute();
      ...
      console.log(\`✅ Authenticated request successful for user: $\{restified.getVariable('globalUserEmail')}\`);
    });

    it('should handle JWT token validation', async function() {
      // Test with valid JWT token (from global auth)
      const validResponse = await restified
        .given()
          .useClient('api')
          // Token automatically applied from global setup
        .when()
          .get('/auth/validate')
          .execute();
      ...
      await validResponse
        .statusCode(200)
        .jsonPath('$.valid', true)
        .jsonPath('$.user.email', (email) => email.includes('@'))
        .execute();
    });

    it('should handle invalid JWT token gracefully', async function() {
      const invalidResponse = await restified
        .given()
          .useClient('api')
          .header('Authorization', 'Bearer invalid-jwt-token-123')
        .when()
          .get('/auth/validate')
          .execute();
      ...
      await invalidResponse
        .statusCode(401)
        .jsonPath('$.error', (error) => error.includes('Invalid') || error.includes('Unauthorized'))
        .execute();
    });

    it('should test JWT token refresh if supported', async function() {
      // This test assumes your API supports token refresh
      const refreshPayload = {
        refreshToken: restified.getVariable('refreshToken') || 'mock-refresh-token'
      };
      ...
      const response = await restified
        .given()
          ...
          .header('Content-Type', 'application/json')
        .when()
          .post('/auth/refresh', refreshPayload)
          .execute();

      // Handle both success and unsupported scenarios  
      // Note: We'll check this in the response validation
      try {
        await response
          .statusCode(200)
          .jsonPath('$.token', (newToken) => {
            expect(newToken).to.be.a('string');
            expect(newToken).to.not.equal(restified.getVariable('globalAuthToken'));
            return true;
          })
          .extract('$.token', 'refreshedToken')
          .execute();
        
        console.log('✅ JWT token refresh successful');
      } catch (error) {
        // API doesn't support refresh or returned error - that's ok for this test
        console.log('ℹ️  Token refresh not supported by API or failed gracefully');
      }
    });
  });
  ...
  describe('Authentication Security', function() {
    it('should reject requests without authentication', async function() {
      const response = await restified
        .given()
          .useClient('api')
          .header('Authorization', '') // Remove auth header
        .when()
          .get('/users/profile')
          .execute();
...
before(async function() {
  ...
  if (config.authentication) {
    ...
    try {
      ...
      const loginPayload = {
        email: config.authentication.credentials.username,
      ...
      const authResponse = await restified
        .given()
          ...
          .header('Content-Type', 'application/json')
        .when()
          [config.authentication.method.toLowerCase() as 'post'](config.authentication.endpoint, loginPayload)
          .execute();
      ...
      if (authToken && config.authentication.autoApplyToClients) {
        ...
        console.log(\`✅ JWT Authentication successful - Token applied to clients\`);
        console.log(\`👤 Authenticated as: $\{restified.getVariable('globalUserEmail')}\`);
      }
    } catch (error: any) {
      console.log('⚠️  JWT Authentication failed, using fallback values');
      console.log(\`   Error: $\{error.message || 'Unknown authentication error'}\`);
      
      if (config.authentication.fallback && config.authentication.fallback.enabled) {
        // Use fallback authentication for CI/CD
        ...
        restified.addAuthTokenToAllClients(config.authentication.fallback.token, config.authentication.authHeaderName || 'Authorization');
        
        console.log('✅ Fallback authentication applied for CI/CD environment');
      } else {
        console.log('❌ Authentication failed and fallback is disabled');
        throw error;
      }
    }
  }
...
# JWT Token Extraction (customize based on your API response)
TOKEN_JSONPATH=$.token
USER_EMAIL_JSONPATH=$.user.email
USER_ID_JSONPATH=$.user.id
EXPIRES_IN_JSONPATH=$.expiresIn

# Authentication Headers
AUTH_HEADER_NAME=Authorization

# Fallback Authentication for CI/CD
FALLBACK_AUTH_ENABLED=true
FALLBACK_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.fallback-jwt-token-for-ci-cd
FALLBACK_EMAIL=ci-cd@company.com
...
try {
  ...
  if (typeof RestifiedHtmlReporter === 'function') {
    ...
    console.log('✅ Using RestifiedHtmlReporter for test reports');
  } else {
  ...
  try {
    ...
    if (typeof RestifiedHtmlReporter === 'function') {
      RestifiedReporter = RestifiedHtmlReporter;
      console.log('✅ Using RestifiedHtmlReporter from local node_modules');
    } else {
      throw new Error('RestifiedHtmlReporter is not a function');
    }
  } catch (error2) {
    try {
      ...
      if (typeof RestifiedHtmlReporter === 'function') {
        RestifiedReporter = RestifiedHtmlReporter;
        console.log('✅ Using RestifiedHtmlReporter from dist/reporting');
      } else {
        throw new Error('RestifiedHtmlReporter is not a function');
      }
    } catch (error3) {
...
if (!RestifiedReporter) {
  console.warn('⚠️  No reporter found, using spec as final fallback');
  RestifiedReporter = require('mocha/lib/reporters/spec');
}
